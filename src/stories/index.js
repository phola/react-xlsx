...
storiesOf('WorkBook', module)
  .addWithInfo('simple usage (no render)', 'dfsdfdsf', () => {
    return simple()
  })
  .addWithInfo('big usage (no render)', 'dfsdfdsf', () => {
    return big()
  })
  .add('wrapped cell', () => {
    return subtest()
  })
  .add('wrapped cell hoc', () => {
    return subtestHOC()
  })
  .addWithInfo('tiny', 'dfsdfdsf', () => {
    const handleXLSX = blob => {
      console.log(blob)
      FileSaver.saveAs(blob, 'formulae.xlsx')
    }
    return <WorkBook><div>ts</div></WorkBook>
  })
  .addWithInfo('with style', 'dfsdfdsf', () => {
    const handleXLSX = blob => {
      console.log(blob)
      FileSaver.saveAs(blob, 'formulae.xlsx')
    }
    return (
      <WorkBook
        ...
        toXLSXCallback={handleXLSX}
        onlyRenderTrigger
      >
        <Cell row={0} col={1} type='n'>0.1</Cell>
        <Cell row={1} col={1} type='b'>false</Cell>
        <Cell row={1} col={2} type='n'>2.3</Cell>
        <Cell cellRef='B3' formula>B1+B2</Cell>
  ...
  .addWithInfo('wrap exisiting Components', 'dfsdfdsf', () => {
    const handleXLSX = blob => {
      console.log(blob)
      FileSaver.saveAs(blob, 'test.xlsx')
    }
    return (
      <WorkBook defaultCellStyle={defaultCellStyle} toXLSXCallback={handleXLSX}>
